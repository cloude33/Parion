...
class DebouncedFilterBar extends StatefulWidget {
  ...
  @override
  State<DebouncedFilterBar> createState() => _DebouncedFilterBarState();
}

class _DebouncedFilterBarState extends State<DebouncedFilterBar> {
  late Debouncer _filterDebouncer;
  bool _isApplyingFilter = false;

  @override
  void initState() {
    super.initState();
    _filterDebouncer = Debouncer(
  ...
  void _debouncedTimeFilterChange(String filter) {
    setState(() {
  ...
  void _debouncedCategoriesChange(List<String> categories) {
    setState(() {
      _isApplyingFilter = true;
    });
  ...
  void _debouncedWalletsChange(List<String> wallets) {
    setState(() {
      _isApplyingFilter = true;
    });

    _filterDebouncer.call(() {
      widget.onWalletsChanged(wallets);
      if (mounted) {
        setState(() {
          _isApplyingFilter = false;
        });
  ...
  void _debouncedTransactionTypeChange(String type) {
    setState(() {
      _isApplyingFilter = true;
    });
        ...
        children: [
          Column(
            ...
            children: [
              ...
              if (hasActiveFilters || widget.selectedCategories.isNotEmpty || widget.selectedWallets.isNotEmpty)
                ...
                children: [
                  _buildFilterButton(
                    ...
                    label: widget.selectedCategories.isEmpty
                        ? 'Kategori'
                        : '${widget.selectedCategories.length} Kategori',
                  ...
                  _buildFilterButton(
                    ...
                    label: widget.selectedWallets.isEmpty
                        ? 'Cüzdan'
                        : '${widget.selectedWallets.length} Cüzdan',
                    isActive: widget.selectedWallets.isNotEmpty,
                  ...
                  _buildFilterButton(
                    ...
                    isActive: widget.selectedTransactionType != 'all',
      ...
      chips.add(_buildRemovableChip(
        ...
        onRemove: () {
          final updated = List<String>.from(widget.selectedCategories)
            ..remove(categoryId);
          _debouncedCategoriesChange(updated);
        },
        isDark: isDark,
      ));
    }
    for (final walletId in widget.selectedWallets) {
      final wallet = widget.availableWallets.firstWhere(
        (w) => w.id == walletId,
        orElse: () => Wallet(
          id: walletId,
          name: walletId,
      ...
      chips.add(_buildRemovableChip(
        ...
        label: wallet.name,
        onRemove: () {
          final updated = List<String>.from(widget.selectedWallets)..remove(walletId);
          _debouncedWalletsChange(updated);
        },
        isDark: isDark,
      ));
    }
  ...
  Widget _buildCustomDateChip(BuildContext context, bool isDark) {
    final isSelected = widget.selectedTimeFilter == 'Özel';
    final dateText = widget.customStartDate != null && widget.customEndDate != null
        ? '${DateFormat('dd/MM').format(widget.customStartDate!)} - ${DateFormat('dd/MM').format(widget.customEndDate!)}'
        : 'Özel';
  ...
  void _showCategoryFilter(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (context) => _CategoryFilterSheet(
        ...
        onChanged: _debouncedCategoriesChange,
      ),
    );
  }
  ...
  void _showWalletFilter(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (context) => _WalletFilterSheet(
        availableWallets: widget.availableWallets,
        selectedWallets: widget.selectedWallets,
        onChanged: _debouncedWalletsChange,
      ),
    );
  }
  ...
  void _showTransactionTypeFilter(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (context) => _TransactionTypeFilterSheet(
        selectedType: widget.selectedTransactionType,
        onChanged: _debouncedTransactionTypeChange,
      ),
    );
  }
}
