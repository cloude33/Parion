...
class _SimpleAuthDebugWidgetState extends State<SimpleAuthDebugWidget> {
  ...
  Future<void> _updateDebugInfo() async {
    ...
    try {
      ...
      final authState = _authOrchestrator.currentAuthState;
      ...
      final isBiometricEnabled = await getIt<IBiometricAuthService>()
          .isBiometricEnabled();
      ...
      final info = {
        ...
        'backgroundLockDelay':
            config.sessionConfig.backgroundLockDelay.inSeconds,
        'sessionTimeout': config.sessionTimeout.inSeconds,
        'unifiedAuthStatus': authState.status.toString().split('.').last,
        'canUseApp': authState.isAuthenticated,
  ...
  Widget _buildDebugInfo() {
    return Column(
      children: [
        _buildInfoRow(
          'Firebase',
          _debugInfo['firebaseAuth']?.toString() ?? 'Unknown',
        ),
        _buildInfoRow(
          'Local',
          _debugInfo['localAuth']?.toString() ?? 'Unknown',
        ),
        _buildInfoRow(
          'Biometric',
          _debugInfo['biometricEnabled']?.toString() ?? 'Unknown',
        ),
        _buildInfoRow(
          'Session',
          _debugInfo['sessionActive']?.toString() ?? 'Unknown',
        ),
        _buildInfoRow(
          'Remaining',
          '${_debugInfo['sessionRemainingSeconds'] ?? 0}s',
        ),
        _buildInfoRow(
          'Status',
          _debugInfo['unifiedAuthStatus']?.toString() ?? 'Unknown',
        ),
        _buildInfoRow(
          'Can Use',
          _debugInfo['canUseApp']?.toString() ?? 'Unknown',
        ),
        _buildInfoRow(
          'Email',
          _debugInfo['firebaseEmail']?.toString() ?? 'None',
        ),
        _buildInfoRow(
          'BG Lock',
          _debugInfo['backgroundLockEnabled']?.toString() ?? 'Unknown',
        ),
  ...
  Future<void> _testBackgroundLock() async {
    ...
    // Simulate background
    await _authOrchestrator.onAppBackground();

    // Wait 2 seconds
    await Future.delayed(const Duration(seconds: 2));

    // Simulate foreground
    await _authOrchestrator.onAppForeground();
    ...
    try {
      ...
      final testConfig = config.copyWith(
        ...
        isBiometricEnabled:
            ...
            true, // Enable biometric if not already enabled
      );
      ...
      final restoreConfig = config.copyWith(sessionTimeout: originalTimeout);
  ...
  Future<void> _enableBiometric() async {
    ...
    try {
      ...
      // Enable biometric authentication through BiometricAuthService
      await getIt<IBiometricAuthService>().enableBiometric();

      debugPrint('✅ Biometric authentication enabled successfully');

      // Test biometric authentication immediately
      final authResult = await getIt<IBiometricAuthService>().authenticate(
        reason: 'Test Biometric',
      );
      if (authResult.isSuccess) {
        debugPrint('✅ Biometric authentication test successful');
      } else {
        debugPrint(
          '⚠️ Biometric authentication test failed: ${authResult.errorMessage}',
        );
      }
    } catch (e) {
