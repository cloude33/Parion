...
class AuthSecureStorageService {
  ...
  ///
  /// [timestamp] - The time when app went to background
  ///
  /// Returns true if storage was successful, false otherwise
  Future<bool> storeBackgroundTimestamp(DateTime timestamp) async {
    try {
      await _ensureInitialized();
      return await _storeSecurely(
      ...
      debugPrint('Failed to store background timestamp: $e');
      return false;
    }
  }
  ...
  ///
  /// Returns the background timestamp, or null if not found
  Future<DateTime?> getBackgroundTimestamp() async {
    try {
      await _ensureInitialized();
      final timeStr = await _retrieveSecurely(_backgroundTimestampKey);
      if (timeStr == null) return null;
      ...
      debugPrint('Failed to get background timestamp: $e');
      return null;
    }
  }
  ...
  ///
  /// Returns true if clearing was successful, false otherwise
  Future<bool> clearBackgroundTimestamp() async {
    try {
      await _ensureInitialized();
      return await _deleteSecurely(_backgroundTimestampKey);
    } catch (e) {
      debugPrint('Failed to clear background timestamp: $e');
      return false;
    }
  }
  ...
  ///
  /// [data] - Verification data to store
  ///
  /// Throws [Exception] if storage fails
  Future<void> storePendingVerification(String data) async {
    try {
      await _ensureInitialized();
      final success = await _storeSecurely(_pendingVerificationKey, data);
      if (!success) {
        throw Exception('Failed to store pending verification');
      }
      debugPrint('Pending verification stored successfully');
    } catch (e) {
  ...
  ///
  /// Returns verification data, or null if not found
  Future<String?> getPendingVerification() async {
    try {
      await _ensureInitialized();
      final result = await _retrieveSecurely(_pendingVerificationKey);
      debugPrint('Pending verification retrieved: ${result != null ? 'found' : 'not found'}');
      return result;
    } catch (e) {
  ...
  Future<void> clearPendingVerification() async {
    try {
      await _ensureInitialized();
      final success = await _deleteSecurely(_pendingVerificationKey);
      if (!success) {
        throw Exception('Failed to clear pending verification');
      }
      debugPrint('Pending verification cleared successfully');
    } catch (e) {
