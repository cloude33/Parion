...
class SecurityProvider extends ChangeNotifier {
  ...
  Future<AuthResult> authenticateForSensitiveOperation({
    ...
    try {
      ...
      return AuthResult.failure(method: method, errorMessage: e.toString());
    } finally {
      _setLoading(false);
    }
  }
  ...
  void _setupStreamListeners() {
    ...
    _authStateSubscription = _authService.authStateStream.listen(
      (authState) {
        ...
        notifyListeners();
        debugPrint(
          'SecurityProvider: Auth state updated: ${authState.isAuthenticated}',
        );
      },
      onError: (error) {
    ...
    _sessionStateSubscription = _sessionManager.sessionStateStream.listen(
      (sessionState) {
        ...
        notifyListeners();
        debugPrint(
          'SecurityProvider: Session state updated: ${sessionState.isActive}',
        );
      },
      onError: (error) {
