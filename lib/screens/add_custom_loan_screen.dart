...
class _AddCustomLoanScreenState extends State<AddCustomLoanScreen> {
  ...
  final TextEditingController _nameController =
      TextEditingController(); // e.g. "Okul Taksidi"
  final TextEditingController _providerController =
      TextEditingController(); // e.g. "Çiğli Final"
  ...
  double get _totalAmount {
    return _items.fold(0.0, (sum, item) {
      final val =
          double.tryParse(
            item.amountController.text.replaceAll('.', '').replaceAll(',', '.'),
          ) ??
          0.0;
      return sum + val;
    });
  ...
  Future<void> _showGenerateDialog() async {
    ...
    if (result == true) {
      ...
      final amount =
          amountController.text; // Keep as string to set controller text
  ...
  Future<void> _saveLoan() async {
    ...
    try {
      ...
      for (int i = 0; i < _items.length; i++) {
        ...
        final isPastDue =
            item.date.isBefore(today) || item.date.isAtSameMomentAs(today);
        ...
        installments.add(
          LoanInstallment(
            ...
            remainingPrincipalAmount:
                0, // Not calculated for custom simple plans
          ),
        );
      }
      ...
      final loan = Loan(
        ...
        bankName: _providerController.text.isEmpty
            ? 'Özel Ödeme'
        ...
        installmentAmount: installments.isEmpty
            ? 0
            : installments.first.amount, // Approximate
                            ...
                            Expanded(
                              child: TextFormField(
                                ...
                                onChanged: (_) => setState(
                                  () {},
                                ), // Trigger rebuild for total calc
                              ),
                            ),
                          ],
                        ),
