...
class _EditTransactionScreenState extends State<EditTransactionScreen> {
  ...
  Future<void> _updateTransaction() async {
    ...
    try {
      // Tutar alanındaki değeri doğru şekilde parse et
      final cleanAmountText = _amountController.text
          .replaceAll('.', '')
          .replaceAll(',', '.');
      final totalAmount = double.tryParse(cleanAmountText) ?? 0.0;
      ...
      children: [
        ...
        TextField(
          ...
          onChanged: (value) {
            ...
            // Sadece rakamları ve virgülü koru
            String cleanValue = value.replaceAll(RegExp(r'[^0-9,]'), '');

            // Birden fazla virgül varsa sadece ilkini tut
            int firstCommaIndex = cleanValue.indexOf(',');
            if (firstCommaIndex != -1 &&
                cleanValue.lastIndexOf(',') != firstCommaIndex) {
              String beforeFirstComma = cleanValue.substring(
                0,
                firstCommaIndex,
              );
              String afterFirstComma = cleanValue.substring(
                firstCommaIndex + 1,
              );
              cleanValue = '$beforeFirstComma,$afterFirstComma'.replaceAll(
                ',',
                '',
              );
            }
