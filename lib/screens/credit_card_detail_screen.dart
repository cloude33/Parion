...
class _CreditCardDetailScreenState extends State<CreditCardDetailScreen> {
  ...
  final DeferredInstallmentService _deferredService =
  ...
  void initState() {
    super.initState();
    // Eğer bugün kesim gününü geçtiyse, varsayılan olarak bir sonraki dönemi (güncel/aktif dönem) göster
    if (DateTime.now().day > widget.card.statementDay) {
  ...
  Future<void> _loadCardDetails() async {
    ...
    try {
      ...
      try {
        cashAdvanceSummary = await _cashAdvanceService.getCashAdvanceSummary(
          widget.card.id,
        );
      } catch (e) {
      ...
      try {
        deferredInstallments = await _deferredService.getDeferredInstallments(
          widget.card.id,
        );
      } catch (e) {
      ...
      final statementDay = widget.card.statementDay;

      // Determine the statement date for the selected month
      // Handle end of month correctly (e.g. if statement day is 31, use 28/29 for Feb)
      final periodEnd = _getStatementDate(now.year, now.month, statementDay);
      final periodStart = _getStatementDate(
        now.year,
        now.month - 1,
        statementDay,
      );
  ...
  DateTime _getStatementDate(int year, int month, int statementDay) {
    // Handle month overflow/underflow for year calculation if needed
    // DateTime constructor handles month 0 or 13 automatically, but for "last day" logic we need strict month
    var d = DateTime(year, month, 1);
    // Find the last day of this month
    var lastDayOfMonth = DateTime(d.year, d.month + 1, 0).day;

    // Clamp statement day
    var day = statementDay;
              ...
              child: ListView(
                ...
                children: [
                  ...
                  if (_rewardSummary != null &&
                      _rewardSummary!['exists'] == true)
                    _buildRewardPointsSection(),
                  if (_rewardSummary != null &&
                      _rewardSummary!['exists'] == true)
                    const SizedBox(height: 16),
                  if (_cashAdvanceSummary != null &&
                      _cashAdvanceSummary!['totalDebt'] > 0)
                  ...
                  if (_deferredInstallments.isNotEmpty)
                    const SizedBox(height: 24),
                  if (_scheduledBills.isNotEmpty) _buildScheduledBillsSection(),
                  if (_scheduledBills.isNotEmpty) const SizedBox(height: 24),
  ...
  Widget _buildCashAdvanceSection() {
    ...
    final totalWithInterest =
        _cashAdvanceSummary!['totalWithInterest'] as double;
  ...
  Widget _buildDeferredInstallmentItem(CreditCardTransaction transaction) {
    final startDate =
        transaction.installmentStartDate ?? transaction.transactionDate;
