...
///
/// Tests:
/// - Yükleme durumunda `StatisticsLoadingState` gösterildiğini doğrula
/// - Boş veri durumunda `StatisticsEmptyState` gösterildiğini doğrula
/// - Hata durumunda `StatisticsErrorState` ve "Tekrar Dene" butonunun gösterildiğini doğrula
...
void main() {
  ...
  group('Özet Sekmesi Widget Testleri (Task 8.2)', () {
    setUp(() async {
    ...
    // ── Requirement 3.8, 14.1: Yükleme durumunda StatisticsLoadingState ──────
    ...
    testWidgets(
      ...
      expect(
        find.byType(StatisticsLoadingState),
        findsWidgets,
        reason:
    ...
    // ── Requirement 3.7, 14.2: Boş veri durumunda StatisticsEmptyState ───────
    ...
    testWidgets(
        'Boş veri durumunda "Veri Bulunamadı" başlığı gösterilmelidir',
        (tester) async {
      GetIt.I.registerSingleton<DataService>(_FakeDataService());
      GetIt.I.registerSingleton<StatisticsService>(_FakeStatisticsService());
      ...
      expect(
        find.text('Veri Bulunamadı'),
        findsOneWidget,
        reason: 'Empty state title must be "Veri Bulunamadı"',
      );
    });

    // ── Requirement 3.9, 14.3: Hata durumunda StatisticsErrorState ───────────

    testWidgets(
        'Hata durumunda StatisticsErrorState gösterilmelidir (Req 3.9, 14.3)',
        (tester) async {
      GetIt.I.registerSingleton<DataService>(_FakeDataService());
      ...
      expect(
        find.byType(StatisticsErrorState),
        findsWidgets,
        reason:
    ...
    testWidgets(
        'Hata durumunda "Tekrar Dene" butonu gösterilmelidir (Req 3.9, 14.3)',
        (tester) async {
      GetIt.I.registerSingleton<DataService>(_FakeDataService());
      GetIt.I.registerSingleton<StatisticsService>(_ErrorStatisticsService());
    ...
    testWidgets(
        '"Tekrar Dene" butonuna basıldığında istisna fırlatılmamalıdır',
        (tester) async {
      GetIt.I.registerSingleton<DataService>(_FakeDataService());
      GetIt.I.registerSingleton<StatisticsService>(_ErrorStatisticsService());
...
/// Futures that never complete — simulates loading state.
...
/// Always throws — simulates error state.
