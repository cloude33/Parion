...
void main() {
  ...
  group('CreditCardService Property Tests', () {
    test(
      'Property 29: Dönem Borcu Hesaplama - Feature: enhanced-credit-card-tracking, Property 29',
      () async {
    ...
    test(
      'Property 30: Harcama Dönem Borcu Ekleme - Feature: enhanced-credit-card-tracking, Property 30',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          final transactionAmount = PropertyTest.randomPositiveDouble(
            min: 10,
            max: 1000,
          );
    ...
    test(
      'Property 31: Eski Dönem Borcu Ödeme Önceliği - Feature: enhanced-credit-card-tracking, Property 31',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          final oldStatementDebt = PropertyTest.randomPositiveDouble(
            min: 100,
            max: 2000,
          );
          ...
          final newStatementDebt = PropertyTest.randomPositiveDouble(
            min: 100,
            max: 2000,
          );
          ...
          final updatedOldStatement = await statementRepo.findById(
            oldStatement.id,
          );
          expect(updatedOldStatement, isNotNull);
          expect(
            updatedOldStatement!.remainingDebt,
            closeTo(oldStatementDebt - paymentAmount, 0.01),
          );

          // Verify new statement debt remains unchanged
          final updatedNewStatement = await statementRepo.findById(
            newStatement.id,
          );
          expect(updatedNewStatement, isNotNull);
          expect(
            updatedNewStatement!.remainingDebt,
            closeTo(oldStatementDebt + newStatementDebt, 0.01),
          );
        }
      },
    );
    ...
    test(
      'Property 32: Toplam Borç Hesaplama - Feature: enhanced-credit-card-tracking, Property 32',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          for (int j = 0; j < statementCount; j++) {
            final statementDebt = PropertyTest.randomPositiveDouble(
              min: 50,
              max: 1000,
            );
