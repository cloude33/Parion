...
void main() {
  ...
  group('KMH Migration Default Values Property Tests', () {
    test(
      'Property 37: KMH accounts without interestRate should get default 24% after migration',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          final walletBeforeMigration = walletsBeforeMigration.firstWhere(
            (w) => w.id == wallet.id,
          );
          expect(
            walletBeforeMigration.interestRate,
            isNull,
            reason:
                'Wallet should not have interestRate before migration (iteration $i)',
          );
          ...
          final migratedWallet = walletsAfterMigration.firstWhere(
            (w) => w.id == wallet.id,
          );
          ...
          expect(
            ...
            equals(24.0),
            reason:
                'Migrated wallet should have default interestRate of 24% (iteration $i)',
          );
          ...
          expect(
            ...
            isNotNull,
            reason:
                'Migrated wallet should have lastInterestDate set (iteration $i)',
          );
          ...
          expect(
            ...
            equals(0.0),
            reason:
                'Migrated wallet should have accruedInterest set to 0 (iteration $i)',
          );
    ...
    test(
      'Property 37: KMH accounts with existing interestRate should preserve their value',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          expect(
            ...
            equals(existingRate),
            reason:
                'Wallet should have custom interestRate before migration (iteration $i)',
          );
          ...
          expect(
            ...
            equals(0),
            reason:
                'Should not migrate account that already has all fields (iteration $i)',
          );

          // Verify the wallet after migration
          final walletsAfterMigration = await dataService.getWallets();
          final unchangedWallet = walletsAfterMigration.firstWhere(
            (w) => w.id == wallet.id,
          );
          ...
          expect(
            ...
            equals(existingRate),
            reason:
                'Wallet should preserve existing interestRate of $existingRate, not change to default (iteration $i)',
          );

          // Property: Other fields should also be preserved
          expect(
            unchangedWallet.lastInterestDate,
            equals(wallet.lastInterestDate),
            reason:
                'Wallet should preserve existing lastInterestDate (iteration $i)',
          );
          expect(
            unchangedWallet.accruedInterest,
            equals(wallet.accruedInterest),
            reason:
                'Wallet should preserve existing accruedInterest (iteration $i)',
          );
    ...
    test(
      'Property 37: Migration should set all missing default values correctly',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          expect(
            migratedWallet.lastInterestDate!.isAfter(
              beforeMigration.subtract(const Duration(seconds: 5)),
            ),
            isTrue,
            reason: 'lastInterestDate should be recent (iteration $i)',
          );
          expect(
            migratedWallet.lastInterestDate!.isBefore(
              afterMigration.add(const Duration(seconds: 5)),
            ),
            isTrue,
            reason:
                'lastInterestDate should not be in the future (iteration $i)',
          );
    ...
    test(
      'Property 37: Migration should handle partial field presence correctly',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          final customDate = DateTime.now().subtract(
          ...
          if (hasInterestRate) {
            expect(
              ...
              equals(customRate),
              reason:
                  'Existing interestRate should be preserved (iteration $i)',
            );
          } else {
            expect(
              migratedWallet.interestRate,
              equals(24.0),
              reason:
                  'Missing interestRate should be set to default 24% (iteration $i)',
            );
          }
          ...
          if (hasLastInterestDate) {
            expect(
              ...
              equals(customDate),
              reason:
                  'Existing lastInterestDate should be preserved (iteration $i)',
            );
          } else {
            expect(
              migratedWallet.lastInterestDate,
          ...
          if (hasAccruedInterest) {
            expect(
              migratedWallet.accruedInterest,
              equals(customAccrued),
              reason:
                  'Existing accruedInterest should be preserved (iteration $i)',
            );
          } else {
            expect(
              migratedWallet.accruedInterest,
              equals(0.0),
              reason:
                  'Missing accruedInterest should be set to 0.0 (iteration $i)',
            );
          }
    ...
    test(
      'Property 37: Multiple KMH accounts should all receive default values correctly',
      () async {
        for (int iteration = 0; iteration < 100; iteration++) {
          ...
          expect(
            ...
            equals(accountCount),
            reason:
                'Should migrate all $accountCount KMH accounts (iteration $iteration)',
          );
          ...
          for (var walletId in walletIds) {
            ...
            expect(
              ...
              equals(24.0),
              reason:
                  'Wallet $walletId should have default interestRate of 24% (iteration $iteration)',
            );
            ...
            expect(
              ...
              isNotNull,
              reason:
                  'Wallet $walletId should have lastInterestDate set (iteration $iteration)',
            );
            ...
            expect(
              ...
              equals(0.0),
              reason:
                  'Wallet $walletId should have accruedInterest set to 0 (iteration $iteration)',
            );
          }
    ...
    test(
      'Property 37: Default values should remain consistent across migration reruns',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          expect(
            walletAfterFirst.interestRate,
            equals(24.0),
            reason:
                'Should have default interestRate after first migration (iteration $i)',
          );
          ...
          expect(
            ...
            reason:
                'interestRate should remain consistent across reruns (iteration $i)',
          );
          expect(
            walletAfterSecond.lastInterestDate,
            equals(firstLastInterestDate),
            reason:
                'lastInterestDate should remain consistent across reruns (iteration $i)',
          );
          expect(
            walletAfterSecond.accruedInterest,
            equals(firstAccruedInterest),
            reason:
                'accruedInterest should remain consistent across reruns (iteration $i)',
          );
