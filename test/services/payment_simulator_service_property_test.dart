...
void main() {
  ...
  group('PaymentSimulatorService Property Tests', () {
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 22: Remaining debt and interest should be correctly calculated',
      generator: () {
        final debt = PropertyTest.randomPositiveDouble(min: 1000, max: 50000);
        final paymentAmount = PropertyTest.randomPositiveDouble(
          min: 100,
          max: debt,
        );
      ...
      property: (data) async {
        ...
        expect(
          (simulation.remainingDebt - expectedRemainingDebt).abs(),
          lessThan(0.01),
        );
        ...
        expect(
          (simulation.interestCharged - expectedInterest).abs(),
          lessThan(0.01),
        );
        ...
        expect(
          (simulation.proposedPayment - paymentAmount).abs(),
          lessThan(0.01),
        );
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 23: Minimum payment interest should be correctly calculated',
      generator: () {
        final debt = PropertyTest.randomPositiveDouble(min: 1000, max: 50000);
        final interestRate = PropertyTest.randomPositiveDouble(
          min: 1.0,
        ...
        return {'debt': debt, 'interestRate': interestRate};
      },
      property: (data) async {
        final debt = data['debt'] as double;
        final interestRate = data['interestRate'] as double;
        ...
        expect(
          (actualRemainingDebt - expectedRemainingDebt).abs(),
          lessThan(0.01),
        );
    ...
    test(
      'Property 23: Minimum payment for small debt should be at least 100',
      () async {
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 24: Early payoff interest savings should be correctly calculated',
      generator: () {
        final debt = PropertyTest.randomPositiveDouble(min: 1000, max: 50000);
        final interestRate = PropertyTest.randomPositiveDouble(
          min: 1.0,
      ...
      property: (data) async {
        ...
        final expectedSavings = minPaymentCost.isFinite
            ? minPaymentCost - fullPaymentCost
            : 0.0;
    ...
    test(
      'Property 24: Early payoff for zero debt should return zero savings',
      () async {
        final card = await createTestCardWithDebt(debt: 0, interestRate: 3.0);
    ...
    test(
      'Property 24: Early payoff should always save interest for significant debt',
      () async {
        final card = await createTestCardWithDebt(
          debt: 10000,
          interestRate: 3.0,
    ...
    test(
      'Property 24: Early payoff savings should increase with higher interest rates',
      () async {
        final debt = 10000.0;

        // Create two cards with different interest rates
        final card1 = await createTestCardWithDebt(
          debt: debt,
          interestRate: 2.0,
        );
        final card2 = await createTestCardWithDebt(
          debt: debt,
          interestRate: 4.0,
        );
    ...
    test(
      'Property 24: Early payoff savings should increase with higher debt',
      () async {
    ...
    test(
      'Property 24: Full payment simulation should match early payoff',
      () async {
        final card = await createTestCardWithDebt(
          debt: 10000,
          interestRate: 3.0,
    ...
    test('Compare payment options should return sorted results', () async {
      ...
      final comparison = await service.comparePaymentOptions(
        card.id,
        paymentAmounts,
      );

      final comparisons =
          comparison['comparisons'] as List<Map<String, dynamic>>;
    ...
    test(
      'Payment recommendation should provide reasonable suggestions',
      () async {
        final card = await createTestCardWithDebt(
          debt: 10000,
          interestRate: 3.0,
        ...
        final recommendedPayment =
            recommendation['recommendedPayment'] as double;
