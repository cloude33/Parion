  ...
  group('KMH Recommendation Criteria Property Tests', () {
    ...
    setUpAll(() async {
      ...
      testDir = await Directory.systemTemp.createTemp(
        'kmh_recommendation_criteria_test_',
      );
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 31: Recommendation must consider both available credit and interest rate',
      generator: () {
      ...
      property: (data) async {
        ...
        if (accountsWithCredit.isNotEmpty) {
          ...
          expect(
            ...
            greaterThan(0),
            reason:
                'Recommended account must have available credit (criterion 1: available credit)',
          );

          // Verify recommended account is from the set of accounts with credit
          expect(
            accountsWithCredit.map((a) => a.id).contains(recommended.id),
            isTrue,
            reason:
                'Recommended account must be from accounts with available credit',
          );
        }
        ...
        if (accountsWithCredit.isNotEmpty && recommended != null) {
          ...
          expect(
            ...
            equals(minRate),
            reason:
                'Recommended account must have lowest interest rate among accounts with credit (criterion 2: interest rate)',
          );
        }

        // Property 3: Accounts without available credit must never be recommended
        final accountsWithoutCredit = kmhAccounts
            .where((a) => a.availableCredit <= 0)
            .toList();
        if (recommended != null && accountsWithoutCredit.isNotEmpty) {
          for (var account in accountsWithoutCredit) {
            expect(
              recommended.id,
              isNot(equals(account.id)),
              reason:
                  'Account without available credit should never be recommended, even if it has lower interest rate',
            );
          }
        }
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 31: Accounts without credit should be excluded even if they have the lowest interest rate',
      generator: () {
        final accounts = [
      ...
      property: (data) async {
        ...
        expect(
          noCredit.interestRate,
          lessThan(hasCredit.interestRate!),
        ...
        expect(
          ...
          reason:
              'Account without credit should NOT be recommended even with lowest rate (available credit criterion must be satisfied first)',
        );
        expect(
          recommended.id,
          equals(hasCredit.id),
          reason:
              'Account with credit should be recommended (both criteria: has credit AND lowest rate among those with credit)',
        );
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 31: When multiple accounts have same rate, available credit amount is the tiebreaker',
      generator: () {
        ...
        final accounts = [
          ...
          // Account 3: Higher rate
          {
            ...
            'creditLimit': 10000.0,
            'interestRate':
                sameRate + PropertyTest.randomPositiveDouble(min: 5, max: 15),
            'lastInterestDate': PropertyTest.randomDateTime(),
            'accruedInterest': PropertyTest.randomPositiveDouble(
              min: 0,
              max: 500,
      ...
      property: (data) async {
        ...
        // Find specific accounts
        final lessCredit = kmhAccounts.firstWhere(
          (a) => a.name == 'Same Rate - Less Credit',
        );
        final moreCredit = kmhAccounts.firstWhere(
          (a) => a.name == 'Same Rate - More Credit',
        );

        // Verify test setup
        expect(
          lessCredit.interestRate,
          equals(sameRate),
          reason: 'Test setup: first account should have the same rate',
        );
        expect(
          moreCredit.interestRate,
          equals(sameRate),
          reason: 'Test setup: second account should have the same rate',
        );
        expect(
          moreCredit.availableCredit,
          greaterThan(lessCredit.availableCredit),
          reason:
              'Test setup: second account should have more available credit',
        );
        ...
        expect(
          ...
          equals(sameRate),
          reason:
              'Recommended should have the lowest rate (which is the same rate)',
        );
        expect(
          recommended.id,
          equals(moreCredit.id),
          reason:
              'When rates are equal, should prefer account with more available credit (both criteria applied: rate AND credit amount)',
        );
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 31: Recommendation logic must evaluate both criteria in correct order (credit availability first, then rate)',
      generator: () {
        final accounts = [
      ...
      property: (data) async {
        ...
        // Find specific accounts
        final noCredit = kmhAccounts.firstWhere(
          (a) => a.name == 'No Credit - Best Rate',
        );
        final mediumRate = kmhAccounts.firstWhere(
          (a) => a.name == 'Has Credit - Medium Rate',
        );
        final worstRate = kmhAccounts.firstWhere(
          (a) => a.name == 'Has Credit - Worst Rate',
        );
        ...
        expect(
          recommended.id,
          equals(mediumRate.id),
          reason:
              'Second criterion: among accounts with credit, must select lowest rate',
        );
        expect(
          recommended.id,
          isNot(equals(worstRate.id)),
          reason:
              'Second criterion: account with higher rate should not be selected',
        );
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 31: Recommendation must be null when no accounts satisfy the available credit criterion',
      generator: () {
      ...
      property: (data) async {
        ...
        expect(
          ...
          isNull,
          reason:
              'When no accounts have available credit (first criterion not satisfied), recommendation must be null',
        );
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 31: Both criteria must be consistently applied across multiple recommendation calls',
      generator: () {
      ...
      property: (data) async {
        ...
        if (recommended1 != null) {
          ...
          expect(
            ...
            reason:
                'Same criteria should produce same recommendation on second call',
          );
          expect(
            recommended3!.id,
            equals(recommended1.id),
            reason:
                'Same criteria should produce same recommendation on third call',
          );

          // Verify both criteria are satisfied in all calls
          expect(
            recommended1.availableCredit,
            greaterThan(0),
            reason:
                'First criterion (available credit) must be satisfied in call 1',
          );
          expect(
            recommended2.availableCredit,
            greaterThan(0),
            reason:
                'First criterion (available credit) must be satisfied in call 2',
          );
          expect(
            recommended3.availableCredit,
            greaterThan(0),
            reason:
                'First criterion (available credit) must be satisfied in call 3',
          );
          ...
          if (accountsWithCredit.isNotEmpty) {
            ...
            expect(
              recommended1.interestRate,
              equals(minRate),
              reason:
                  'Second criterion (lowest rate) must be satisfied in call 1',
            );
            expect(
              recommended2.interestRate,
              equals(minRate),
              reason:
                  'Second criterion (lowest rate) must be satisfied in call 2',
            );
            expect(
              recommended3.interestRate,
              equals(minRate),
              reason:
                  'Second criterion (lowest rate) must be satisfied in call 3',
            );
          }
        } else {
          expect(
