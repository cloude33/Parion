...
void main() {
  group('Auth Services Integration Tests', () {
    ...
    setUpAll(() {
      ...
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(const MethodChannel('local_auth'), (
    ...
    group('Service Initialization and Coordination', () {
      test('should initialize all services successfully', () async {
        ...
        expect(
          authOrchestrator.currentAuthState.status,
      ...
      test('should coordinate auth state changes across services', () async {
        ...
        final authSubscription = authOrchestrator.authStateStream.listen(
          authStateChanges.add,
        );
        final sessionSubscription = sessionManager.sessionStateStream.listen(
          sessionStateChanges.add,
        );
    ...
    group('Cross-Service Error Handling', () {
      test(
        'should handle network errors consistently across services',
        () async {
      ...
      test('should handle biometric service errors with fallback', () async {
        ...
        final credentials = {'reason': 'Test authentication'};
      ...
      test('should handle social login errors appropriately', () async {
        ...
        final credentials = {'provider': 'google'};
    ...
    group('Session Management Integration', () {
      ...
      test('should handle session expiration correctly', () async {
        ...
        await authOrchestrator.authenticate(
          AuthMethod.emailPassword,
          credentials,
        );
    ...
    group('Security Event Logging Integration', () {
      test('should log security events for all auth operations', () async {
        ...
        final subscription = securityController.securityEventsStream.listen(
          securityEvents.add,
        );
        ...
        final attemptEvent = securityEvents.firstWhere(
          (e) =>
              e.type ==
              SecurityEventType
                  .pinVerified, // Using existing type as placeholder
        );
    ...
    group('Background/Foreground State Handling', () {
      ...
      test(
        'should require re-authentication after background timeout',
        () async {
          // Set short background timeout
          securityController.setBackgroundLockDelay(
...
class MockSecurityController implements ISecurityController {
  ...
  final StreamController<SecurityEvent> _eventsController =
  ...
  @override
  Future<String?> retrieveSecurely(String key, {bool encrypted = true}) async =>
      null;
  ...
  void setShouldFailSessionCreation(bool fail) =>
      _shouldFailSessionCreation = fail;
  void setRequiresSensitiveAuth(bool requires) =>
      _requiresSensitiveAuth = requires;
