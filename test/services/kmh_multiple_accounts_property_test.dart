...
void main() {
  group('KMH Multiple Accounts Listing Property Tests', () {
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 28: All KMH accounts should be listed without skipping any',
      generator: () {
        ...
        final nonKmhAccounts = List.generate(nonKmhAccountCount, (index) {
          ...
          return {
            'id': const Uuid().v4(),
            'name':
                'Regular Account ${index + 1} - ${PropertyTest.randomString(minLength: 5, maxLength: 15)}',
            'balance': PropertyTest.randomDouble(min: -10000, max: 100000),
            'type': selectedType,
            'color':
                '#${PropertyTest.randomInt(min: 0, max: 0xFFFFFF).toRadixString(16).padLeft(6, '0')}',
            'icon': selectedType == 'cash'
        ...
        return {'kmhAccounts': kmhAccounts, 'nonKmhAccounts': nonKmhAccounts};
      },
      property: (data) async {
        ...
        final nonKmhAccountsData =
            data['nonKmhAccounts'] as List<Map<String, dynamic>>;
        ...
        final kmhAccountsRetrieved = retrievedWallets
            .where((w) => w.isKmhAccount)
            .toList();

        // Property 1: All KMH accounts should be retrieved
        expect(
          kmhAccountsRetrieved.length,
          equals(kmhAccountsData.length),
        ...
        for (var accountData in kmhAccountsData) {
          final found = kmhAccountsRetrieved.any(
        ...
        final expectedIds = kmhAccountsData
        ...
        for (var accountData in nonKmhAccountsData) {
          final foundInKmh = kmhAccountsRetrieved.any(
            (w) => w.id == accountData['id'],
          );
          expect(
            foundInKmh,
            isFalse,
            reason:
                'Non-KMH account ${accountData['id']} should not be in KMH list',
          );
        }
        ...
        for (var kmhAccount in kmhAccountsRetrieved) {
          ...
          expect(
            kmhAccount.creditLimit,
            greaterThan(0),
            reason:
                'KMH account ${kmhAccount.id} should have positive credit limit',
          );
        }

        // Property 6: Total wallet count should equal KMH + non-KMH accounts
        final totalExpected =
            kmhAccountsData.length + nonKmhAccountsData.length;
        expect(
          retrievedWallets.length,
          equals(totalExpected),
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 28: Empty KMH account list when no KMH accounts exist',
      generator: () {
        ...
        return {'nonKmhAccounts': nonKmhAccounts};
      },
      property: (data) async {
        // Clear wallets before each iteration
        final prefs = await dataService.getPrefs();
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 28: KMH accounts with different balances (positive and negative) should all be listed',
      generator: () {
      ...
      property: (data) async {
        ...
        expect(
          kmhAccountsRetrieved.length,
          equals(accountsData.length),
          reason:
              'All KMH accounts should be listed regardless of positive or negative balance',
        );

        // Property 2: Both positive and negative balance accounts should be present
        final positiveBalanceAccounts = kmhAccountsRetrieved
            .where((w) => w.balance > 0)
            .toList();
        final negativeBalanceAccounts = kmhAccountsRetrieved
            .where((w) => w.balance < 0)
            .toList();

        // At least one of each should exist based on our generator
        expect(
          positiveBalanceAccounts.isNotEmpty ||
              negativeBalanceAccounts.isNotEmpty,
          isTrue,
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 28: KMH accounts with varying credit limits should all be listed',
      generator: () {
        final accountCount = PropertyTest.randomInt(min: 2, max: 8);
      ...
      property: (data) async {
        ...
        expect(
          ...
          equals(accountsData.length),
          reason:
              'All KMH accounts should be listed regardless of credit limit amount',
        );
        ...
        for (var accountData in accountsData) {
          final found = kmhAccountsRetrieved.firstWhere(
            (w) => w.id == accountData['id'],
          );
          expect(
            found.creditLimit,
            equals(accountData['creditLimit']),
            reason:
                'Credit limit should be preserved for account ${accountData['id']}',
          );
        }
