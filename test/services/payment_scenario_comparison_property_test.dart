...
void main() {
  group('Payment Scenario Comparison Property Tests', () {
    ...
    PropertyTest.forAll<Map<String, double>>(
      description:
          'property: generated scenarios are comparable and have valid duration and cost',
      generator: () {
        ...
        // Annual rate: 15% - 40%
        final annualRate = PropertyTest.randomPositiveDouble(
          min: 15.0,
          max: 40.0,
        );

        return {'debt': debt, 'annualRate': annualRate};
      },
      property: (data) {
    ...
    PropertyTest.forAll<Map<String, double>>(
      description:
          'property: scenarios with higher monthly payments have lower total interest',
      generator: () {
        final debt = PropertyTest.randomPositiveDouble(
          min: 10000.0,
          max: 40000.0,
    ...
    PropertyTest.forAll<Map<String, double>>(
      description:
          'property: scenario comparison provides mathematically correct metrics',
      generator: () {
        final debt = PropertyTest.randomPositiveDouble(
          min: 8000.0,
          max: 35000.0,
      ...
      property: (data) {
        ...
        // Monthly difference
        final expectedMonthlyDiff =
            scenario2.monthlyPayment - scenario1.monthlyPayment;
        if ((comparison['monthlyDifference'] - expectedMonthlyDiff).abs() >
            tolerance) {
          return false;
        }

        // Duration difference
        final expectedDurationDiff =
            scenario2.durationMonths - scenario1.durationMonths;
        if (comparison['durationDifference'] != expectedDurationDiff) {
          return false;
        }

        // Interest difference
        final expectedInterestDiff =
            scenario2.totalInterest - scenario1.totalInterest;
        if ((comparison['interestDifference'] - expectedInterestDiff).abs() >
            tolerance) {
          return false;
        }

        // Total difference
        final expectedTotalDiff =
            scenario2.totalPayment - scenario1.totalPayment;
        if ((comparison['totalDifference'] - expectedTotalDiff).abs() >
            tolerance) {
          return false;
        }
        ...
        if (scenario2.totalPayment > 0) {
          final expectedPercentage =
              ...
              100);
          if ((comparison['percentageSavings'] - expectedPercentage).abs() >
              tolerance) {
            return false;
          }
        }
    ...
    PropertyTest.forAll<Map<String, double>>(
      ...
      property: (data) {
        ...
        final minScenario = scenarios.reduce(
          (a, b) => a.monthlyPayment < b.monthlyPayment ? a : b,
        );
    ...
    PropertyTest.forAll<Map<String, double>>(
      ...
      property: (data) {
        ...
        final aggressiveScenario = scenarios.reduce(
          (a, b) => a.monthlyPayment > b.monthlyPayment ? a : b,
        );
