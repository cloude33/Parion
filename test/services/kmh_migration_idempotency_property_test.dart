...
void main() {
  ...
  group('KMH Migration Idempotency Property Tests', () {
    test(
      'Property 40: Running migration multiple times should only migrate on first run',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          expect(
            result3.accountsMigrated,
            equals(0),
            reason:
                'Third migration should not migrate accounts again - idempotency (iteration $i)',
          );
    ...
    test(
      'Property 40: Migration flag should be set after first run and persist',
      () async {
        for (int i = 0; i < 100; i++) {
          // Create a KMH account
          final wallet = Wallet(
          ...
          expect(
            ...
            isFalse,
            reason:
                'Migration should not be completed before first run (iteration $i)',
          );
          ...
          final statusAfterSecond = await migrationService
              .isMigrationCompleted();
          expect(
            statusAfterSecond,
            isTrue,
            reason:
                'Migration flag should persist after second run (iteration $i)',
          );
    ...
    test(
      'Property 40: Wallet data should not be modified on subsequent migration runs',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          final walletAfterFirst = walletsAfterFirst.firstWhere(
            (w) => w.id == wallet.id,
          );
          ...
          expect(
            ...
            isNotNull,
            reason:
                'Wallet should have interestRate after first migration (iteration $i)',
          );
          expect(
            firstLastInterestDate,
            isNotNull,
            reason:
                'Wallet should have lastInterestDate after first migration (iteration $i)',
          );
          expect(
            firstAccruedInterest,
            isNotNull,
            reason:
                'Wallet should have accruedInterest after first migration (iteration $i)',
          );
          ...
          final walletAfterSecond = walletsAfterSecond.firstWhere(
            (w) => w.id == wallet.id,
          );

          // Property: All wallet fields should remain unchanged (idempotency)
          expect(
            walletAfterSecond.interestRate,
            equals(firstInterestRate),
            reason:
                'interestRate should not change on second migration (iteration $i)',
          );
          expect(
            walletAfterSecond.lastInterestDate,
            equals(firstLastInterestDate),
            reason:
                'lastInterestDate should not change on second migration (iteration $i)',
          );
          expect(
            walletAfterSecond.accruedInterest,
            equals(firstAccruedInterest),
            reason:
                'accruedInterest should not change on second migration (iteration $i)',
          );
          expect(
            walletAfterSecond.balance,
            equals(firstBalance),
            reason:
                'balance should not change on second migration (iteration $i)',
          );
          expect(
            walletAfterSecond.creditLimit,
            equals(firstCreditLimit),
            reason:
                'creditLimit should not change on second migration (iteration $i)',
          );
          ...
          final walletAfterThird = walletsAfterThird.firstWhere(
            (w) => w.id == wallet.id,
          );

          // Property: All wallet fields should still remain unchanged
          expect(
            walletAfterThird.interestRate,
            equals(firstInterestRate),
            reason:
                'interestRate should not change on third migration (iteration $i)',
          );
          expect(
            walletAfterThird.lastInterestDate,
            equals(firstLastInterestDate),
            reason:
                'lastInterestDate should not change on third migration (iteration $i)',
          );
          expect(
            walletAfterThird.accruedInterest,
            equals(firstAccruedInterest),
            reason:
                'accruedInterest should not change on third migration (iteration $i)',
          );
    ...
    test(
      'Property 40: KmhTransactions should not be duplicated on subsequent migration runs',
      () async {
        for (int i = 0; i < 100; i++) {
          // Create a KMH account
          final wallet = Wallet(
          ...
          expect(
            result1.transactionsConverted,
            equals(transactionCount),
            reason:
                'First migration should convert all transactions (iteration $i)',
          );

          // Get KmhTransactions after first migration
          final kmhTransactionsAfterFirst = await kmhRepository.getTransactions(
            wallet.id,
          );
          ...
          expect(
            ...
            equals(transactionCount),
            reason:
                'Should have $transactionCount KmhTransactions after first migration (iteration $i)',
          );
          ...
          expect(
            result2.transactionsConverted,
            equals(0),
            reason:
                'Second migration should not convert transactions again (iteration $i)',
          );

          // Get KmhTransactions after second migration
          final kmhTransactionsAfterSecond = await kmhRepository
          ...
          expect(
            ...
            reason:
                'KmhTransactions should not be duplicated on second migration (iteration $i)',
          );
          ...
          expect(
            result3.transactionsConverted,
            equals(0),
            reason:
                'Third migration should not convert transactions again (iteration $i)',
          );

          // Get KmhTransactions after third migration
          final kmhTransactionsAfterThird = await kmhRepository.getTransactions(
            wallet.id,
          );
          ...
          expect(
            ...
            equals(countAfterFirst),
            reason:
                'KmhTransactions should not be duplicated on third migration (iteration $i)',
          );
    ...
    test(
      'Property 40: Multiple KMH accounts should not be re-migrated on subsequent runs',
      () async {
        for (int iteration = 0; iteration < 100; iteration++) {
          ...
          expect(
            ...
            equals(accountCount),
            reason:
                'First migration should migrate all $accountCount accounts (iteration $iteration)',
          );

          // Get all wallets after first migration
          final walletsAfterFirst = await dataService.getWallets();
          final kmhAccountsAfterFirst = walletsAfterFirst
              .where((w) => w.isKmhAccount)
              .toList();

          // Property: All accounts should be KMH accounts with fields set
          expect(
            kmhAccountsAfterFirst.length,
            equals(accountCount),
            reason:
                'All accounts should be KMH accounts after first migration (iteration $iteration)',
          );
          ...
          for (var account in kmhAccountsAfterFirst) {
            expect(
              ...
              isNotNull,
              reason:
                  'Account ${account.name} should have interestRate (iteration $iteration)',
            );
          }
          ...
          expect(
            ...
            equals(0),
            reason:
                'Second migration should not migrate accounts again (iteration $iteration)',
          );

          // Get all wallets after second migration
          final walletsAfterSecond = await dataService.getWallets();
          final kmhAccountsAfterSecond = walletsAfterSecond
              .where((w) => w.isKmhAccount)
              .toList();

          // Property: Number of KMH accounts should remain the same
          expect(
            kmhAccountsAfterSecond.length,
            equals(accountCount),
            reason:
                'Number of KMH accounts should not change (iteration $iteration)',
          );

          // Property: All account data should remain unchanged
          for (int i = 0; i < accountCount; i++) {
            final accountFirst = kmhAccountsAfterFirst[i];
            final accountSecond = kmhAccountsAfterSecond.firstWhere(
              (w) => w.id == accountFirst.id,
            );

            expect(
              accountSecond.interestRate,
              equals(accountFirst.interestRate),
              reason:
                  'Account ${accountFirst.name} interestRate should not change (iteration $iteration)',
            );
            expect(
              accountSecond.lastInterestDate,
              equals(accountFirst.lastInterestDate),
              reason:
                  'Account ${accountFirst.name} lastInterestDate should not change (iteration $iteration)',
            );
            expect(
              accountSecond.accruedInterest,
              equals(accountFirst.accruedInterest),
              reason:
                  'Account ${accountFirst.name} accruedInterest should not change (iteration $iteration)',
            );
          }
    ...
    test(
      'Property 40: Migration status should correctly reflect completion state',
      () async {
        for (int i = 0; i < 100; i++) {
          // Create a KMH account
          final wallet = Wallet(
          ...
          expect(
            statusBefore.migrationDate,
            isNull,
            reason:
                'Migration date should be null before first run (iteration $i)',
          );
          ...
          expect(
            statusAfterFirst.isCompleted,
            isTrue,
            reason:
                'Migration should be completed after first run (iteration $i)',
          );
          expect(
            statusAfterFirst.migrationDate,
            isNotNull,
            reason:
                'Migration date should be set after first run (iteration $i)',
          );
          ...
          expect(
            statusAfterSecond.isCompleted,
            isTrue,
            reason:
                'Migration should still be completed after second run (iteration $i)',
          );

          // Property: Migration date should not change
          expect(
            statusAfterSecond.migrationDate,
            equals(firstMigrationDate),
            reason:
                'Migration date should not change on subsequent runs (iteration $i)',
          );
    ...
    test(
      'Property 40: Idempotency should work even with mixed account types',
      () async {
        for (int iteration = 0; iteration < 100; iteration++) {
          ...
          expect(
            ...
            equals(kmhCount),
            reason:
                'First migration should migrate only $kmhCount KMH accounts (iteration $iteration)',
          );
          ...
          expect(
            totalCountAfterFirst,
            equals(kmhCount + nonKmhCount),
            reason:
                'Total wallet count should be ${kmhCount + nonKmhCount} (iteration $iteration)',
          );
          ...
          expect(
            ...
            equals(0),
            reason:
                'Second migration should not migrate accounts (iteration $iteration)',
          );
          ...
          expect(
            totalCountAfterSecond,
            equals(totalCountAfterFirst),
            reason:
                'Total wallet count should not change (iteration $iteration)',
          );

          // Property: Non-KMH accounts should remain unchanged
          final nonKmhAccountsAfterSecond = walletsAfterSecond
              .where((w) => !w.isKmhAccount)
              .toList();
          expect(
            nonKmhAccountsAfterSecond.length,
            equals(nonKmhCount),
            reason:
                'Non-KMH account count should remain $nonKmhCount (iteration $iteration)',
          );
    ...
    test(
      'Property 40: Idempotency should be maintained even after app restart simulation',
      () async {
        for (int i = 0; i < 100; i++) {
          // Create a KMH account
          final wallet = Wallet(
          ...
          expect(
            ...
            equals(0),
            reason:
                'Migration should be idempotent even after service restart (iteration $i)',
          );
          ...
          expect(
            ...
            isTrue,
            reason:
                'Migration should still be marked as completed after restart (iteration $i)',
          );
    ...
    test(
      'Property 40: Idempotency should handle edge case of zero KMH accounts',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          expect(
            ...
            equals(0),
            reason:
                'First migration should migrate 0 accounts when no KMH accounts exist (iteration $i)',
          );

          // Property: Migration should be marked as completed even with no accounts
          final isCompletedAfterFirst = await migrationService
              .isMigrationCompleted();
          expect(
            isCompletedAfterFirst,
            isTrue,
            reason:
                'Migration should be marked as completed even with no KMH accounts (iteration $i)',
          );
          ...
          expect(
            ...
            equals(0),
            reason:
                'Second migration should migrate 0 accounts (idempotency) (iteration $i)',
          );
