...
import 'package:parion/services/cash_advance_service.dart';
...
void main() {
  ...
  group('CashAdvanceService Property Tests', () {
    ...
    test(
      'Property 25: Nakit Avans İşaretleme - For any cash advance transaction, system should mark it as cash advance',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          final description =
              'Nakit Avans ${PropertyTest.randomInt(min: 1, max: 1000)}';
    ...
    test(
      'Property 26: Nakit Avans Faiz Oranı - For any cash advance, system should apply cash advance interest rate',
      () async {
        for (int i = 0; i < 100; i++) {
          // Generate random test data
          final cardId = uuid.v4();
          ...
          final cashAdvanceRate = PropertyTest.randomPositiveDouble(
            min: 3.0,
            max: 10.0,
          );
          ...
          expect(
            summary['cashAdvanceRate'],
            equals(cashAdvanceRate),
            reason:
                'Cash advance should use cash advance rate, not monthly rate',
          );
    ...
    test(
      'Property 27: Günlük Nakit Avans Faizi - For any cash advance debt, system should calculate daily interest',
      () async {
        for (int i = 0; i < 100; i++) {
          // Generate random test data
          final cardId = uuid.v4();
          ...
          expect(
            (interest - expectedInterest).abs(),
            lessThan(0.01),
            reason:
                'Interest should be calculated as: amount * daily_rate * days',
          );
    ...
    test(
      'Property 28: Nakit Avans Ödeme Önceliği - For any payment, system should prioritize cash advance debt',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          // Generate random test data
          final cardId = uuid.v4();
          final cashAdvanceAmount = PropertyTest.randomPositiveDouble(
            min: 100,
            max: 2000,
          );
          final regularAmount = PropertyTest.randomPositiveDouble(
            min: 100,
            max: 2000,
          );
          ...
          final cashAdvances = await service.getCashAdvances(cardId);
          final totalCashAdvanceDebt = await service.getTotalCashAdvanceDebt(
            cardId,
          );
          ...
          expect(cashAdvances.first.isCashAdvance, isTrue);
          expect(
            totalCashAdvanceDebt,
            equals(cashAdvanceAmount),
          ...
          final allTransactions = await CreditCardBoxService
              .transactionsBox
              .values
          ...
          final nonCashAdvances = allTransactions
              .where((t) => !t.isCashAdvance)
              .toList();
          expect(
            nonCashAdvances.length,
            equals(1),
