...
void main() {
  ...
  setUpAll(() async {
    ...
    final testPath =
        './test_hive_limit_${DateTime.now().millisecondsSinceEpoch}';
    Hive.init(testPath);
  ...
  group('LimitAlertService Property Tests', () {
    test(
      'Property 4: Limit Kullanım Vurgulama - Feature: enhanced-credit-card-tracking, Property 4',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          final creditLimit =
              1000.0 + random.nextDouble() * 9000.0; // 1000-10000
          ...
          final utilization = await service.calculateUtilizationPercentage(
            card.id,
          );
          ...
          if (utilization >= 80.0) {
            expect(
              ...
              isTrue,
              reason:
                  'Card with ${utilization.toStringAsFixed(2)}% utilization should show warning',
            );
          } else {
            expect(
              shouldWarn,
              isFalse,
              reason:
                  'Card with ${utilization.toStringAsFixed(2)}% utilization should not show warning',
            );
          }
    ...
    test(
      'Property 17: %80 Limit Uyarısı - Feature: enhanced-credit-card-tracking, Property 17',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          final alert80After = alertsAfter.firstWhere(
            (a) => a.threshold == 80.0,
          );
          ...
          if (utilization >= 80.0) {
            expect(
              alert80After.isTriggered,
              isTrue,
              reason:
                  'Alert should be triggered at ${utilization.toStringAsFixed(2)}% utilization',
            );
            expect(
              alert80After.triggeredAt,
              isNotNull,
            ...
            expect(
              ...
              isFalse,
              reason:
                  'Alert should not be triggered at ${utilization.toStringAsFixed(2)}% utilization',
            );
          }
    ...
    test(
      'Property 18: %90 Limit Uyarısı - Feature: enhanced-credit-card-tracking, Property 18',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          // Re-fetch alert after potential update
          final alertsAfter = await alertRepo.findByCardId(card.id);
          final alert90After = alertsAfter.firstWhere(
            (a) => a.threshold == 90.0,
          );
          ...
          if (utilization >= 90.0) {
            expect(
              alert90After.isTriggered,
              isTrue,
              reason:
                  'Alert should be triggered at ${utilization.toStringAsFixed(2)}% utilization',
            );
            expect(
              alert90After.triggeredAt,
              isNotNull,
              reason: 'Triggered alert should have triggeredAt timestamp',
            );
          } else {
    ...
    test(
      'Property 19: %100 Limit Uyarısı - Feature: enhanced-credit-card-tracking, Property 19',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          final utilizationTarget =
              95.0 + random.nextDouble() * 10.0; // 95-105%
          final debt = (utilizationTarget / 100) * creditLimit;
          ...
          // Re-fetch alert after potential update
          final alertsAfter = await alertRepo.findByCardId(card.id);
          final alert100After = alertsAfter.firstWhere(
            (a) => a.threshold == 100.0,
          );
          ...
          if (utilization >= 100.0) {
            expect(
              alert100After.isTriggered,
              isTrue,
              reason:
                  'Alert should be triggered at ${utilization.toStringAsFixed(2)}% utilization',
            );
            expect(
              alert100After.triggeredAt,
              isNotNull,
              reason: 'Triggered alert should have triggeredAt timestamp',
            );
          } else {
    ...
    test(
      'Property 20: Limit Uyarısı İçeriği - Feature: enhanced-credit-card-tracking, Property 20',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          final utilizationTarget =
              80.0 + random.nextDouble() * 20.0; // 80-100%
          final debt = (utilizationTarget / 100) * creditLimit;
          ...
          expect(
            (actualAvailable - expectedAvailable).abs(),
            lessThan(0.01),
    ...
    test(
      'Property 21: Ödeme Sonrası Limit Güncelleme - Feature: enhanced-credit-card-tracking, Property 21',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          final alert80Before = alertsBefore.firstWhere(
            (a) => a.threshold == 80.0,
          );
          ...
          final alert80AfterTrigger = alertsAfterTrigger.firstWhere(
            (a) => a.threshold == 80.0,
          );
          expect(
            alert80AfterTrigger.isTriggered,
            isTrue,
          ...
          final utilizationAfter = await service.calculateUtilizationPercentage(
            card.id,
          );
          final alertsAfter = await alertRepo.findByCardId(card.id);
          final alert80After = alertsAfter.firstWhere(
