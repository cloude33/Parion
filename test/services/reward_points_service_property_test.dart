...
void main() {
  ...
  group('RewardPointsService Property Tests', () {
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 9: Conversion rate should be correctly stored and used in calculations',
      generator: () {
        final rewardTypes = ['bonus', 'worldpuan', 'miles', 'cashback'];
        return {
          'cardId': const Uuid().v4(),
          'rewardType':
              rewardTypes[PropertyTest.randomInt(
                min: 0,
                max: rewardTypes.length - 1,
              )],
          'conversionRate': PropertyTest.randomPositiveDouble(
            min: 0.001,
            max: 10.0,
          ),
          'pointsBalance': PropertyTest.randomPositiveDouble(
            min: 0,
            max: 100000,
          ),
        };
    ...
    test(
      'Property 9: Zero or negative conversion rate should be rejected',
      () async {
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 10: Points should be automatically calculated for transaction amounts',
      generator: () {
        final rewardTypes = ['bonus', 'worldpuan', 'miles', 'cashback'];
        return {
          'cardId': const Uuid().v4(),
          ...
          'transactionAmount': PropertyTest.randomPositiveDouble(
            min: 0.01,
            max: 100000,
          ),
        };
      ...
      property: (data) async {
        ...
        expect(
          (newBalance - initialBalance - calculatedPoints).abs(),
          lessThan(0.0001),
        );
    ...
    test(
      'Property 10: Zero or negative amounts should return zero points',
      () async {
        final cardId = const Uuid().v4();
        await service.initializeRewards(cardId, 'bonus', 1.0);

        // Test zero amount
        final zeroPoints = await service.calculatePointsForTransaction(
          cardId,
          0,
        );
        ...
        final negativePoints = await service.calculatePointsForTransaction(
          cardId,
          -100,
        );
    ...
    test(
      'Property 10: Points calculation should work for cards without reward system',
      () async {
        final cardId = const Uuid().v4();
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 11: Points balance invariant should hold after spending points',
      generator: () {
        final rewardTypes = ['bonus', 'worldpuan', 'miles', 'cashback'];
        ...
        final pointsToSpend = PropertyTest.randomPositiveDouble(
          min: 1,
          max: initialBalance,
        );
      ...
      property: (data) async {
        ...
        final spendingTransactions = transactions
        ...
        expect(
          (lastSpending.pointsSpent - pointsToSpend).abs(),
          lessThan(0.0001),
        );
    ...
    test('Property 11: Cannot spend more points than available', () async {
      ...
      expect(() => service.spendPoints(cardId, 200, 'Test'), throwsException);
    ...
    test(
      'Property 11: Multiple operations should maintain balance invariant',
      () async {
        final cardId = const Uuid().v4();
        await service.initializeRewards(cardId, 'bonus', 1.0);
    ...
    test('Property 11: Zero points spending should be rejected', () async {
      ...
      expect(() => service.spendPoints(cardId, 0, 'Test'), throwsException);
    });
    ...
    test('Property 11: Negative points spending should be rejected', () async {
      ...
      expect(() => service.spendPoints(cardId, -10, 'Test'), throwsException);
    });
  });
}
