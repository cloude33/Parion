...
void main() {
  ...
  group('Payment History Property Tests', () {
    test(
      'Property 51: Ödeme Kaydı - Payment records all required fields correctly',
      () async {
    ...
    test(
      'Property 52: Ödeme Borç Azaltma Invariant\'ı - New debt equals old debt minus payment',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          final paymentAmount =
              random.nextDouble() * oldDebt; // Payment can't exceed debt
          ...
          expect(
            payment.remainingDebtAfterPayment,
            closeTo(expectedNewDebt, 0.01),
          );
          expect(
            payment.remainingDebtAfterPayment,
            closeTo(oldDebt - paymentAmount, 0.01),
          );
    ...
    test(
      'Property 53: Ödeme Kronolojik Sıralama - Payments are sorted chronologically',
      () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          for (int k = 0; k < sortedPayments.length - 1; k++) {
            expect(
              sortedPayments[k].paymentDate.isBefore(
                    sortedPayments[k + 1].paymentDate,
                  ) ||
                  sortedPayments[k].paymentDate.isAtSameMomentAs(
                    sortedPayments[k + 1].paymentDate,
                  ),
              isTrue,
              reason:
                  'Payment at index $k should be before or equal to payment at index ${k + 1}',
            );
          }
