...
void main() {
  ...
  test('checkAndGenerateStatements generates statement when due', () async {
    ...
    expect(statements, isNotEmpty, reason: 'Statement should be generated for today');
  ...
  test('checkAndGenerateStatements does NOT generate statement when NOT due', () async {
    final cardRepo = CreditCardRepository();
    final statementRepo = CreditCardStatementRepository();
    final generator = StatementGeneratorService();
    ...
    // Assert
    final statements = await statementRepo.findByCardId(card.id);
    expect(statements, isEmpty, reason: 'Statement should NOT be generated for future date');
  });
}
