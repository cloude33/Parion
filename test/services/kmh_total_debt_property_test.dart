...
void main() {
  group('KMH Total Debt Calculation Property Tests', () {
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 21: Total KMH debt equals sum of all negative balances',
      generator: () {
      ...
      property: (data) async {
        ...
        final positiveBalanceAccounts = retrievedWallets.where(
          (w) => w.isKmhAccount && w.balance >= 0,
        );
        ...
        for (var account in positiveBalanceAccounts) {
          ...
          final debtWithoutThisAccount = retrievedWallets
              .where(
                (w) => w.isKmhAccount && w.balance < 0 && w.id != account.id,
              )
              .fold(0.0, (sum, w) => sum + w.balance.abs());
        ...
        final negativeBalanceAccounts = retrievedWallets.where(
          (w) => w.isKmhAccount && w.balance < 0,
        );
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 21: Total debt calculation with mixed account types',
      generator: () {
        ...
        final loans = List.generate(loanCount, (index) {
          return {
            'id': const Uuid().v4(),
            'name': 'Loan ${index + 1}',
            'remainingAmount': PropertyTest.randomPositiveDouble(
              min: 1000,
              max: 100000,
            ),
          };
      ...
      property: (data) async {
        ...
        final expectedCreditCardDebt = creditCardsData.fold(
          0.0,
          (sum, a) => sum + (a['balance'] as double).abs(),
        );

        final expectedLoanDebt = loansData.fold(
          0.0,
        ...
        final actualLoanDebt = retrievedLoans.fold(
          0.0,
          (sum, loan) => sum + loan.remainingAmount,
        );

        final actualTotalDebt =
            actualKmhDebt + actualCreditCardDebt + actualLoanDebt;
        ...
        expect(
          ...
          reason:
              'Total debt should equal sum of KMH + credit card + loan debts',
        );

        // Property 3: KMH debt should be a component of total debt
        expect(
          actualTotalDebt,
          greaterThanOrEqualTo(actualKmhDebt),
        ...
        for (var bank in allBankAccounts) {
          if (bank.creditLimit == 0 && bank.balance < 0) {
            ...
            expect(
              ...
              isFalse,
              reason:
                  'Regular bank account ${bank.id} should not be counted as KMH debt',
            );
          }
        }
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      description:
          'Property 21: KMH debt with all positive balances should be zero',
      generator: () {
        final kmhAccountCount = PropertyTest.randomInt(min: 1, max: 10);
      ...
      property: (data) async {
        ...
        expect(
          ...
          equals(0.0),
          reason:
              'KMH debt should be zero when all accounts have positive balances',
        );
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      ...
      property: (data) async {
        ...
        final expectedDebt = kmhAccountsData.fold(
          0.0,
          (sum, a) => sum + (a['balance'] as double).abs(),
        );
        ...
        expect(
          ...
          equals(expectedDebt),
          reason:
              'KMH debt should equal sum of all negative balances when all are negative',
        );
        ...
        expect(
          ...
          greaterThan(0),
          reason:
              'KMH debt should be positive when accounts have negative balances',
        );
    ...
    PropertyTest.forAll<Map<String, dynamic>>(
      ...
      property: (data) async {
        ...
        expect(
          ...
          equals(expectedDebt),
          reason:
              'Single account debt should equal its absolute balance if negative, 0 otherwise',
        );
