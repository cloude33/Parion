...
void main() {
  ...
  group('KMH Transaction Migration Property Tests', () {
    test(
      'Property 38: Transactions for KMH accounts should be converted to KmhTransaction',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          for (int j = 0; j < transactionCount; j++) {
            ...
            final transactionType =
                transactionTypes[PropertyTest.randomInt(
                  min: 0,
                  max: transactionTypes.length - 1,
                )];
            ...
            final transaction = Transaction(
              ...
              date: DateTime.now().subtract(
                Duration(days: PropertyTest.randomInt(min: 0, max: 365)),
              ),
          ...
          expect(
            result.transactionsConverted,
            equals(transactionCount),
            reason:
                'Should convert all $transactionCount transactions (iteration $i)',
          );

          // Verify KmhTransactions were created
          final kmhTransactions = await kmhRepository.getTransactions(
            wallet.id,
          );

          // Property: Number of KmhTransactions should match original transaction count
          expect(
            kmhTransactions.length,
            equals(transactionCount),
            reason:
                'Should create $transactionCount KmhTransactions (iteration $i)',
          );

          // Property: Each KmhTransaction should have a linkedTransactionId
          for (var kmhTransaction in kmhTransactions) {
            expect(
              kmhTransaction.linkedTransactionId,
              isNotNull,
              reason:
                  'KmhTransaction should have linkedTransactionId (iteration $i)',
            );
            expect(
              transactionIds.contains(kmhTransaction.linkedTransactionId),
              isTrue,
              reason:
                  'linkedTransactionId should reference an original transaction (iteration $i)',
            );
          }
          ...
          for (var kmhTransaction in kmhTransactions) {
            expect(
              kmhTransaction.walletId,
              equals(wallet.id),
              reason:
                  'KmhTransaction should have correct walletId (iteration $i)',
            );
          }
    ...
    test(
      'Property 38: Transaction type mapping should be correct (expense -> withdrawal, income -> deposit)',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          expect(
            convertedExpense.type,
            equals(KmhTransactionType.withdrawal),
            reason:
                'Expense transaction should be converted to withdrawal (iteration $i)',
          );

          // Property: Income should be converted to deposit
          expect(
            convertedIncome.type,
            equals(KmhTransactionType.deposit),
            reason:
                'Income transaction should be converted to deposit (iteration $i)',
          );
    ...
    test(
      'Property 38: Transaction amounts should be preserved (as absolute values)',
      () async {
        for (int i = 0; i < 100; i++) {
          // Create a KMH account
          final wallet = Wallet(
          ...
          final amount1 = PropertyTest.randomPositiveDouble(
            min: 100,
            max: 5000,
          );
          final amount2 = PropertyTest.randomPositiveDouble(
            min: 100,
            max: 5000,
          );
          ...
          expect(
            ...
            equals(amount1.abs()),
            reason:
                'Transaction amount should be preserved as absolute value (iteration $i)',
          );
          expect(
            converted2.amount,
            equals(amount2.abs()),
    ...
    test(
      'Property 38: Transaction dates and descriptions should be preserved',
      () async {
        for (int i = 0; i < 100; i++) {
          // Create a KMH account
          final wallet = Wallet(
          ...
          final transactionDescription = PropertyTest.randomString(
            minLength: 10,
            maxLength: 100,
          );
          ...
          expect(
            kmhTransaction.description,
            equals(transactionDescription),
            reason:
                'Transaction description should be preserved (iteration $i)',
          );
    ...
    test(
      'Property 38: Transactions for non-KMH accounts should NOT be converted',
      () async {
        for (int i = 0; i < 100; i++) {
          ...
          expect(
            ...
            equals(0),
            reason:
                'Should not convert transactions for non-KMH accounts (iteration $i)',
          );
          ...
          expect(
            ...
            equals(0),
            reason:
                'Should not create KmhTransactions for non-KMH accounts (iteration $i)',
          );
    ...
    test(
      'Property 38: Migration should handle multiple KMH accounts with transactions correctly',
      () async {
        for (int iteration = 0; iteration < 100; iteration++) {
          ...
          expect(
            ...
            reason:
                'Should convert all transactions from all KMH accounts (iteration $iteration)',
          );
          ...
          for (var data in walletData) {
            ...
            expect(
              ...
              reason:
                  'Wallet ${wallet.name} should have $expectedCount KmhTransactions (iteration $iteration)',
            );

            // Property: All KmhTransactions should link to original transactions
            for (var kmhTransaction in kmhTransactions) {
              expect(
                originalIds.contains(kmhTransaction.linkedTransactionId),
                isTrue,
                reason:
                    'KmhTransaction should link to original transaction (iteration $iteration)',
              );
            }
          }
    ...
    test(
      'Property 38: Migration should be idempotent - transactions should not be duplicated on rerun',
      () async {
        for (int i = 0; i < 100; i++) {
          // Create a KMH account
          final wallet = Wallet(
          ...
          expect(
            ...
            reason:
                'KmhTransactions should not be duplicated on migration rerun (iteration $i)',
          );
    ...
    test(
      'Property 38: Transfer transactions should be converted to transfer type',
      () async {
        for (int i = 0; i < 100; i++) {
          // Create a KMH account
          final wallet = Wallet(
          ...
          expect(
            kmhTransaction.type,
            equals(KmhTransactionType.transfer),
            reason:
                'Transfer transaction should be converted to transfer type (iteration $i)',
          );
