# 💰 Pario - Kişisel Finans Uygulaması

Modern Flutter tabanlı kişisel bütçe, kredi kartı ve fatura takip uygulaması.

[![Download APK](https://img.shields.io/badge/Download-APK-green.svg)](https://github.com/cloude33/Pario/releases/latest)
[![Flutter](https://img.shields.io/badge/Flutter-3.10+-blue.svg)](https://flutter.dev/)
[![License](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Version](https://img.shields.io/badge/Version-2.0.0-brightgreen.svg)](https://github.com/cloude33/Pario/releases/tag/v2.0.0)

## 📥 İndir

[**APK İndir (v2.0.0)**](https://github.com/cloude33/Pario/releases/download/v2.0.0/Pario-v2.0.0.apk)
...
### 💾 Veri Yönetimi
- **Bulut Yedekleme**: Firebase ile güvenli bulut depolama
  - **Unified Authentication**: Firebase hesabı zorunlu, güvenli kimlik doğrulama
  - **Çapraz Platform Desteği**: Android ↔ iOS veri senkronizasyonu
  - **Otomatik Yedekleme**: Günlük otomatik bulut yedeklemesi
  - **Çoklu Yedek**: Birden fazla yedek noktası saklama
  - **Güvenli Şifreleme**: Veriler şifrelenmiş olarak saklanır
  - **Hızlı Geri Yükleme**: Tek tıkla tüm verileri geri yükleme
- **Yerel Yedekleme**: Cihazda yerel yedek oluşturma
...
1. Projeyi klonlayın:
```bash
git clone https://github.com/cloude33/Pario.git
cd Pario
```

2. **iOS için otomatik kurulum scripti çalıştırın:**
```bash
./setup_ios.sh
```
...
4. iOS bağımlılıklarını yükleyin:
```bash
cd ios
pod install
cd ..
...
5. Build runner'ı çalıştırın:
```bash
flutter pub run build_runner build --delete-conflicting-outputs
```

### Her GitHub Pull Sonrası

GitHub'dan her proje çektiğinizde aşağıdaki hatalarla karşılaşabilirsiniz:
...
**Çözüm:** Otomatik kurulum scriptini çalıştırın:
```bash
./setup_ios.sh
```

Bu script şunları yapar:
- Flutter bağımlılıklarını temizler ve yeniden yükler
- iOS Pods'ları yeniden yükler
...
- `firebase_core`: Firebase temel yapı
- `firebase_auth`: Kullanıcı kimlik doğrulama
- `cloud_firestore`: NoSQL bulut veritabanı
- `firebase_storage`: Dosya depolama
- `firebase_analytics`: Kullanım analitikleri
...
│   ├── cloud_backup_screen.dart        # Bulut yedekleme yönetimi
...
│   ├── data_service.dart
│   ├── backup_service.dart             # Bulut yedekleme servisi
│   ├── auto_backup_service.dart        # Otomatik yedekleme
...
## ☁️ Bulut Yedekleme Kullanımı

### İlk Kurulum
1. **Ayarlar** → **Veri Yönetimi** → **Bulut Yedekleme**
2. Firebase hesabınızla oturum açın (Google ile giriş veya email/şifre)
3. **Yedekle** butonuna tıklayarak ilk yedeğinizi oluşturun

### Güvenlik Sistemi
- **Unified Authentication**: Kullanıcılar önce Firebase hesabı oluşturmalı
- **Biyometrik Opsiyonel**: Firebase hesabı olan kullanıcılar biyometrik kimlik doğrulamayı etkinleştirebilir
- **Güvenli Yedekleme**: Sadece Firebase hesabı olan kullanıcılar yedekleme yapabilir

### Otomatik Yedekleme
1. Bulut yedekleme ekranında **Otomatik Yedekleme** seçeneğini açın
2. Verileriniz her gün otomatik olarak buluta yedeklenecek
3. Son yedekleme tarihi ekranda görüntülenir

### Cihazlar Arası Senkronizasyon
1. **Android'den iPhone'a geçiş**:
   - iPhone'da aynı Firebase hesabıyla giriş yapın
   - **Geri Yükle** butonuna tıklayın
   - Android verileriniz iPhone'a aktarılır

2. **iPhone'dan Android'e geçiş**:
   - Android'de aynı Firebase hesabıyla giriş yapın
   - **Geri Yükle** butonuna tıklayın
   - iOS verileriniz Android'e aktarılır

### Yedek Yönetimi
- **Çoklu Yedek**: Birden fazla yedek noktası saklanır
- **Yedek Silme**: Eski yedekleri manuel olarak silebilirsiniz
- **Boyut Kontrolü**: Yedek dosya boyutu ve içeriği görüntülenir
- **Güvenlik**: Tüm veriler şifrelenmiş olarak saklanır

### Güvenlik Özellikleri
- **Unified Authentication**: Firebase hesabı zorunlu, biyometrik opsiyonel
- **Firebase Auth**: Google kimlik doğrulama veya email/şifre
- **Şifreleme**: Veriler AES-256 ile şifrelenir
- **Erişim Kontrolü**: Sadece hesap sahibi verilerine erişebilir
- **Otomatik Temizlik**: Eski yedekler otomatik silinir (7 adet saklanır)
...
Proje Linki: [https://github.com/cloude33/Pario](https://github.com/cloude33/Pario)
